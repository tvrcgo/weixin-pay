...
WXPay.mix('redpack', function (opts, fn) {
  opts.mch_billno =  `${this.options.mch_id}${moment().format("YYYYMMDD")}${Date.now().toString().substr(2, 10)}`
  opts.wxappid = this.wxpayID.appid
  opts.mch_id = this.wxpayID.mch_id
  opts.nonce_str = opts.nonce_str || util.generateNonceString();
  opts.sign = this.sign(opts);
  console.log(this.wxpayID, opts)
  request({
